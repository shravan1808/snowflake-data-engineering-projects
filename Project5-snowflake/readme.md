...
A multinational retail company has successfully completed the 

dimensional modeling phase of its enterprise Data Warehouse. 

The management now requires the design team to implement a Star 

Schema that can efficiently support Business Intelligence (BI) and 

Online Analytical Processing (OLAP) queries.



The organization receives sales transactions daily from multiple 

retail branches located across India. Business analysts frequently 

generate reports such as Top Customers, Top Products, Monthly Revenue,

Branch Performance, State-wise Sales, Category-wise Revenue, Customer 

Purchase Trends, and Product Performance Dashboards.



To improve query performance and simplify analytical reporting, the 

Data Warehouse Architect has decided to organize the warehouse using 

the Star Schema architecture.



As a Data Warehouse Designer, your responsibility is to identify the 

Fact Table, Dimension Tables, Primary Keys, Foreign Keys, measures, 

and relationships, and then design a complete Star Schema capable of 

supporting all required analytical reports.
...
The Star Schema should support the following reports:
...
Phase-1: Analyze the Business Requirements
...
Phase-3: Identify Dimension Tables

\----------

For every Dimension Table identify
...
Phase-4: Design the Star Schema
...
Explain why Star Schema is suitable for
...
A customer purchases one or more products from a retail branch on a 
...
| FACT\_SALES | Stores all sales transactions generated by retail branches. |
...
| Date\_ID      | Foreign Key   |

| Quantity     | Measure       |

| Total\_Amount | Measure       |
...
| DIM\_CUSTOMER    | Stores customer information |

| DIM\_PRODUCT     | Stores product information  |

| DIM\_BRANCH      | Stores branch information   |

| DIM\_DATE        | Stores date information     |
...
One record in FACT\_SALES represents the sale of one product purchased by one customer from one retail branch on one specific date.
...
DIM\_CUSTOMER (1) --------< FACT\_SALES (M)



DIM\_PRODUCT  (1) --------< FACT\_SALES (M)



DIM\_BRANCH   (1) --------< FACT\_SALES (M)



DIM\_DATE     (1) --------< FACT\_SALES (M)
...
| Central Table     | FACT\_SALES                   |

| Dimension Tables  | 4                            |

| Normalization     | No (Denormalized Dimensions) |

| Query Performance | High                         |

| Joins Required    | Minimum                      |

| Best Suitable For | OLAP \& Business Intelligence |
...
Comparison with Operational Database
...
| Product\_ID (PK)      |---| Sale\_ID (PK)         |---| Date\_ID (PK)         |

| Product\_Name         |   | Customer\_ID (FK)     |   | Date                 |

| Category             |   | Product\_ID (FK)      |   | Day                  |

| Brand                |   | Branch\_ID (FK)       |   | Day\_Name             |

| Price                |   | Date\_ID (FK)         |   | Week\_No              |

+----------------------+   | Quantity             |   | Month                |

&#x20;                          | Total\_Amount         |   | Quarter              |
